...
async function createEngineDJDatabase(
  outputPath: string,
  playlists: { name: string; tracks: PlaylistTrack[] }[]
) {
  // Create Engine Library directory structure
  const engineLibPath = path.join(outputPath, 'Engine Library');
  ...
  for (const [csvPath, track] of trackMap) {
    ...
    const expectedFilename = path.basename(csvPath);
    ...
    trackIdMap.set(csvPath, result.lastInsertRowid as number);
    trackNum++;
...
async function main() {
  ...
  try {
    ...
    const trackCount = db.prepare('SELECT COUNT(*) as count FROM Track').get() as { count: number };
    const playlistCount = db.prepare('SELECT COUNT(*) as count FROM Playlist').get() as { count: number };
