...
async function testExport() {
  ...
  try {
    ...
    if (result.success) {
      ...
      const dbPath = path.join(testOutputPath, 'Engine Library', 'm.db');
      try {
        ...
        const metadataCount = db.prepare('SELECT COUNT(*) as count FROM MetaData').get() as { count: number };
        ...
        // Sample a track
        const sampleTrack = db.prepare('SELECT * FROM Track LIMIT 1').get() as any;
        if (sampleTrack) {
          console.log(`\n  🎵 Sample Track:`);
